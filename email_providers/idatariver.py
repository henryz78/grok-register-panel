"""iDataRiver Temp Mail API (cbea) 临时邮箱提供商。

API 文档参考：https://www.idatariver.com/project/temp-mail-api-cbea
认证方式：通过 query 参数传递 apikey。
业务响应：HTTP 状态码多为 200，顶层 code=0 或 200 表示成功。
核心接口：
  - GET {base}/generate/v1        生成临时邮箱
  - GET {base}/messages/v1        获取指定邮箱的邮件列表 (id=email_id)
  - GET {base}/message/detail/v1  获取指定邮件详情 (id=msg_id)

直连约定：请求时显式传递 proxies={}，确保不走注册代理。
"""

from __future__ import annotations

import base64
import re
import threading
import time
from typing import Any, Callable, List, Optional, Tuple
from urllib.parse import urlparse

from email_providers.common import extract_verification_code, generate_username

HttpGet = Callable[..., Any]
HttpPost = Callable[..., Any]
HttpDelete = Callable[..., Any]

API_BASE_DEFAULT = "https://apiok.us/api/cbea"
API_TIMEOUT = 65

_ADDRESS_KEYS = ("email", "emailAddress", "address", "mail")
_ID_KEYS = ("id", "emailId", "email_id", "mailId")


def reset_runtime_state() -> None:
    """重置运行时状态（测试与重载使用）。"""
    pass


def normalize_base(base_url: str = "") -> str:
    """规范化 iDataRiver API 基地址。"""
    raw = str(base_url or "").strip()
    if not raw:
        raw = API_BASE_DEFAULT
    if "://" not in raw:
        raw = "https://" + raw
    parsed = urlparse(raw)
    origin = f"{parsed.scheme}://{parsed.netloc}".rstrip("/")
    path = (parsed.path or "").rstrip("/")

    for ep in ("/generate/v1", "/message/detail/v1", "/messages/v1"):
        if path.endswith(ep):
            path = path[: -len(ep)].rstrip("/")
            break

    if path.endswith("/cbea"):
        return f"{origin}{path}"
    if path.endswith("/api"):
        return f"{origin}{path}/cbea"
    if path:
        return f"{origin}{path}/api/cbea"
    return f"{origin}/api/cbea"


def _api(base_url: str, path: str) -> str:
    base = normalize_base(base_url)
    if not path.startswith("/"):
        path = "/" + path
    return f"{base}{path}"


def _params(api_key: str, **extra: Any) -> dict:
    params = {"apikey": str(api_key or "").strip()}
    params.update({k: v for k, v in extra.items() if v is not None})
    return params


def _headers() -> dict:
    return {"Accept": "application/json"}


def _decode_b64(text: str) -> str:
    try:
        padded = text + "=" * (-len(text) % 4)
        return base64.b64decode(padded.encode("ascii")).decode("utf-8", errors="ignore").strip()
    except Exception:
        return ""


def _b64_encode(text: str) -> str:
    try:
        return base64.b64encode(text.encode("utf-8")).decode("ascii").strip()
    except Exception:
        return text


def _raise_http(resp, action: str) -> None:
    status = int(getattr(resp, "status_code", 0) or 0)
    if status >= 400:
        detail = str(getattr(resp, "text", "") or "")[:300]
        raise Exception(f"iDataRiver {action} 失败 HTTP {status}: {detail or 'unknown'}")


def _parse_json(resp, action: str) -> Any:
    try:
        return resp.json()
    except Exception as exc:
        preview = str(getattr(resp, "text", "") or "")[:300]
        raise Exception(f"iDataRiver {action} 返回非 JSON: {preview}") from exc


def _unwrap_payload(data: Any, action: str = "请求") -> Any:
    """解包 iDataRiver 响应，校验业务 code 与获取业务实体。"""
    if not isinstance(data, dict):
        return data

    code = data.get("code")
    if code is not None:
        try:
            code_num = int(code)
            if code_num not in (0, 200):
                msg = (
                    data.get("message")
                    or data.get("msg")
                    or data.get("error")
                    or data.get("errorMessage")
                    or str(data)
                )
                raise Exception(f"iDataRiver {action} 业务失败 code={code_num}: {msg}")
        except ValueError:
            pass

    if data.get("success") is False:
        msg = data.get("message") or data.get("msg") or data.get("error") or str(data)
        raise Exception(f"iDataRiver {action} 业务失败: {msg}")

    if any(k in data for k in ("id", "email", "address", "messages", "message", "emails")):
        return data

    for k in ("result", "data", "response", "payload"):
        payload = data.get(k)
        if isinstance(payload, (dict, list)) and payload:
            return payload

    return data


def create_mailbox(
    http_get: HttpGet,
    http_post: Optional[HttpPost] = None,
    base_url: str = "",
    api_key: str = "",
    *,
    domain: str = "",
    username: str = "",
    **kwargs: Any,
) -> Tuple[str, str]:
    """GET /generate/v1 → (email_address, email_id)。"""
    del http_post, domain, username, kwargs
    base = normalize_base(base_url)
    key = str(api_key or "").strip()
    if not base:
        raise Exception("iDataRiver API 地址未配置（idatariver_api_base）")
    if not key:
        raise Exception("iDataRiver API Key 未配置（idatariver_api_key）")

    resp = http_get(
        _api(base, "/generate/v1"),
        params=_params(key, type="*"),
        headers=_headers(),
        timeout=API_TIMEOUT,
        proxies={},
    )
    _raise_http(resp, "创建邮箱")
    data = _unwrap_payload(_parse_json(resp, "创建邮箱"), "创建邮箱")
    if isinstance(data, list):
        data = data[0] if data else {}
    if not isinstance(data, dict):
        raise Exception(f"iDataRiver 创建邮箱响应异常: {data!r}")

    email_id = ""
    for k in _ID_KEYS:
        val = str(data.get(k) or "").strip()
        if val:
            email_id = val
            break

    address = ""
    for k in _ADDRESS_KEYS:
        val = str(data.get(k) or "").strip()
        if val:
            address = val
            break

    # 若 address 被打码（如 xx***@domain），尝试从 base64 格式的 email_id 恢复
    decoded = _decode_b64(email_id) if email_id else ""
    if "@" in decoded:
        if "*" in address or not address:
            address = decoded
    if not email_id and address:
        email_id = _b64_encode(address)

    if not address:
        raise Exception(f"iDataRiver 返回邮箱地址为空: {data}")
    if not email_id:
        raise Exception(f"iDataRiver 返回邮箱 id 为空: {data}")

    return address, email_id


def list_messages(
    http_get: HttpGet,
    base_url: str,
    email_id: str,
    api_key: str,
) -> List[dict]:
    """GET /messages/v1?id={email_id} → 邮件列表。"""
    base = normalize_base(base_url)
    key = str(api_key or "").strip()
    eid = str(email_id or "").strip()
    if not base or not key or not eid:
        return []

    resp = http_get(
        _api(base, "/messages/v1"),
        params=_params(key, id=eid),
        headers=_headers(),
        timeout=API_TIMEOUT,
        proxies={},
    )
    _raise_http(resp, "获取邮件列表")
    data = _unwrap_payload(_parse_json(resp, "获取邮件列表"), "获取邮件列表")
    if isinstance(data, list):
        return [item for item in data if isinstance(item, dict)]
    if isinstance(data, dict):
        for k in ("list", "messages", "emails", "results", "data"):
            val = data.get(k)
            if isinstance(val, list):
                return [item for item in val if isinstance(item, dict)]
    return []


def get_message_detail(
    http_get: HttpGet,
    base_url: str,
    message_id: str,
    api_key: str,
) -> dict:
    """GET /message/detail/v1?id={message_id} → 邮件正文。"""
    base = normalize_base(base_url)
    key = str(api_key or "").strip()
    mid = str(message_id or "").strip()
    if not base or not key or not mid:
        return {}

    resp = http_get(
        _api(base, "/message/detail/v1"),
        params=_params(key, id=mid),
        headers=_headers(),
        timeout=API_TIMEOUT,
        proxies={},
    )
    _raise_http(resp, "获取邮件详情")
    data = _unwrap_payload(_parse_json(resp, "获取邮件详情"), "获取邮件详情")
    if isinstance(data, list):
        data = data[0] if data else {}
    return data if isinstance(data, dict) else {}


def _message_text(detail: dict, subject: str = "") -> Tuple[str, str]:
    parts: List[str] = []
    subj = str(subject or detail.get("subject") or "")
    for field in ("text", "html", "body", "content", "raw"):
        value = detail.get(field)
        if isinstance(value, str) and value.strip():
            parts.append(re.sub(r"<[^>]+>", " ", value))
        elif isinstance(value, dict):
            for subfield in ("text", "html", "content"):
                subval = value.get(subfield)
                if isinstance(subval, str) and subval.strip():
                    parts.append(re.sub(r"<[^>]+>", " ", subval))
    return subj, "\n".join(parts)


def wait_for_code(
    http_get: HttpGet,
    base_url: str,
    api_key: str,
    email_id: str,
    *,
    email: str = "",
    timeout: int = 180,
    poll_interval: int = 4,
    http_delete: Optional[HttpDelete] = None,
    raise_if_cancelled: Callable[[Optional[Callable[[], bool]]], None],
    sleep_with_cancel: Callable[[float, Optional[Callable[[], bool]]], None],
    log_callback: Optional[Callable[[str], None]] = None,
    cancel_callback: Optional[Callable[[], bool]] = None,
    resend_callback: Optional[Callable[[], None]] = None,
) -> str:
    """轮询 iDataRiver 获取 xAI 验证码。"""
    del http_delete
    base = normalize_base(base_url)
    key = str(api_key or "").strip()
    eid = str(email_id or "").strip()
    if not base:
        raise Exception("iDataRiver API 地址未配置（idatariver_api_base）")
    if not key:
        raise Exception("iDataRiver API Key 未配置（idatariver_api_key）")
    if not eid:
        raise Exception("iDataRiver 邮箱 ID 为空")

    deadline = time.time() + timeout
    seen_attempts: dict[str, int] = {}
    next_resend_at = time.time() + 35

    while time.time() < deadline:
        raise_if_cancelled(cancel_callback)
        if resend_callback and time.time() >= next_resend_at:
            try:
                resend_callback()
                if log_callback:
                    log_callback("[*] 已触发重新发送验证码")
            except Exception as exc:
                if log_callback:
                    log_callback(f"[Debug] 触发重发验证码失败: {exc}")
            next_resend_at = time.time() + 35

        try:
            messages = list_messages(http_get, base, eid, key)
        except Exception as exc:
            if log_callback:
                log_callback(f"[Debug] iDataRiver 拉取邮件列表失败: {exc}")
            sleep_with_cancel(poll_interval, cancel_callback)
            continue

        if log_callback:
            log_callback(f"[Debug] iDataRiver 收到邮件数: {len(messages)}")

        for msg in messages:
            msg_id = ""
            for k in _ID_KEYS:
                val = str(msg.get(k) or "").strip()
                if val:
                    msg_id = val
                    break
            if not msg_id:
                continue

            attempt = seen_attempts.get(msg_id, 0)
            if attempt >= 5:
                continue
            seen_attempts[msg_id] = attempt + 1

            list_subject = str(msg.get("subject") or "")
            code = extract_verification_code(list_subject, list_subject)
            if code:
                if log_callback:
                    log_callback(f"[*] iDataRiver 从主题提取到验证码: {code}")
                return code

            try:
                detail = get_message_detail(http_get, base, msg_id, key)
            except Exception as exc:
                if log_callback:
                    log_callback(f"[Debug] iDataRiver 获取邮件详情失败: {exc}")
                continue

            subject, combined = _message_text(detail, list_subject)
            code = extract_verification_code(combined, subject or list_subject)
            if code:
                if log_callback:
                    log_callback(f"[*] iDataRiver 从正文提取到验证码: {code}")
                return code

        sleep_with_cancel(poll_interval, cancel_callback)

    raise Exception(f"iDataRiver 在 {timeout}s 内未收到验证码邮件（邮箱: {email or eid}）")
